...
| 护城河 | 第三步：护城河评估 | 巴菲特 |
| 风险 | 第四步：逆向思考与风险清单 | 芒格 |
| 管理层 | 第五步：管理层评估 | 段永平+巴菲特 |
| 行业趋势 | 第六步：行业与文明趋势 | 李录 |
| 估值 | 第七步：估值与安全边际 | 巴菲特+段永平 |
| 财务数据 | 第一步：数据收集 | 通用 |
| 竞争格局 | 第一步：数据收集 + 第三步：护城河 | 通用 |
| 综合判断 | 第八步：综合决策备忘录 | 通用 |

5. **在报告中优先引用本地信息**，标注"📌本地信息源"标签，注明记录日期和可信度
6. 本地信息已覆盖的维度，减少网络搜索深度，标注"已有本地记录"
7. 本地信息缺失的维度，标注"待本地补充"，通过网络搜索填充

**本地信息与网络信息的优先级**：
- 一手观察 > 行业观察 > 财报数据 > 券商研报 > 媒体报道
- 本地已记录的信息不需要重复网络搜索

**如果本地无信息**：记录"本地信息源：无"，继续正常研究流程。研究完成后，建议用户使用 `/local-info` 录入关键发现，标注对应的分析维度。
