...
**要求：** 三情景的EPS和PE假设必须有明确依据（历史数据、行业对比、周期位置）；必须使用 `tools/financial_rigor.py three-scenario` 精确计算目标价，禁止心算；表格下方注明"数据来源：tools/financial_rigor.py 三情景估值模型"
...
**计算方法：** 通过求解方程 `P_悲观 × 悲观目标价 + P_中性 × 中性目标价 + P_乐观 × 乐观目标价 = 当前股价` 反推概率权重，约束条件 `P_悲观 + P_中性 + P_乐观 = 100%`。提供2-3组不同的概率权重组合解读。

**分析要点：**
- 当前PE处于历史什么位置？这告诉你市场在定价什么周期阶段
