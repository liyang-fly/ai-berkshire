...
产出**三情景价格映射表**，明确写出每个情景假设下的目标价：

| 情景 | 核心假设 | 预期EPS | 目标PE | 目标价 | 较当前涨跌 |
|------|---------|--------|-------|-------|-----------|
| 悲观 | 行业下行、盈利恶化 | XX元 | XXx | **XX元** | -XX% |
| 合理(中性) | 正常经营、稳定增长 | XX元 | XXx | **XX元** | ±X% |
| 乐观 | 行业上行、超预期 | XX元 | XXx | **XX元** | +XX% |

**要求：**
- 三情景的EPS和PE假设必须有明确依据（历史数据、行业对比、锂价/行业周期位置）
- 必须使用 `tools/financial_rigor.py three-scenario` 精确计算目标价，禁止心算
- 在表格下方注明"数据来源：tools/financial_rigor.py 三情景估值模型，使用精确十进制计算"

#### 当前价格映射（隐含预期概率分析）

产出**当前股价隐含的市场预期概率分析**，回答"当前股价反映了市场多大比例的悲观/合理/乐观预期"：
...
**计算方法：**
1. 使用三情景目标价作为三个锚点
2. 通过求解方程 `P_悲观 × 悲观目标价 + P_中性 × 中性目标价 + P_乐观 × 乐观目标价 = 当前股价` 反推概率权重
3. 约束条件：`P_悲观 + P_中性 + P_乐观 = 100%`
4. 提供2-3组不同的概率权重组合，分别对应"保守""中性""激进"三种解读
5. 结合行业周期位置、公司基本面趋势，给出推荐的概率权重解读

**分析要点：**
- 当前PE(或PB)处于历史什么位置？这告诉你市场在定价什么周期阶段
- 如果当前股价接近乐观目标价 → 市场已充分定价利好，安全边际不足
- 如果当前股价接近悲观目标价 → 市场过度悲观，可能是逆向投资机会
- 如果当前股价介于中性与乐观之间 → 市场预期温和复苏，需验证基本面是否支撑
